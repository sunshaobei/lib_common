 ...
 */
public class NestedHeadOverScrollView extends NestedScrollView implements OverScrollView,
    ...
    public NestedHeadOverScrollView(Context context) {
        this(context, null);
    }

    public NestedHeadOverScrollView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NestedHeadOverScrollView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean handleOverScroll(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
                        ...
                        switch (mOverScrollEdge) {
                            case OVERSCROLL_EDGE_TOP: {
                                if (headView != null) {
                                    ...
                                    int newHeight = (int) (height + deltaY);

                                    if (deltaY < 0 && newHeight < headOriginHeight) {
                                        newHeight = headOriginHeight;
                                    }
            ...
            case MotionEvent.ACTION_UP:
                if (headView != null) {
                    if (mOverScrollEdge == OVERSCROLL_EDGE_TOP || mOverScrollEdge == OVERSCROLL_EDGE_TOP_OR_BOTTOM) {
                        headAnimator(headView.getHeight(), headOriginHeight, DURATION_SPRING_BACK);
                    }
                }
    ...
    private void headAnimator(int start, int end, int duration) {
        if (isHeadAnimation) return;
        if (start == end) {
            isHeadAnimation = false;
            return;
        }
        ValueAnimator headRelaseAnimator = ValueAnimator.ofInt(start, end);
        headRelaseAnimator.setDuration(duration);
        headRelaseAnimator.setInterpolator(mInterpolator);
        headRelaseAnimator.addUpdateListener(animation -> {
            ViewGroup.LayoutParams layoutParams = headView.getLayoutParams();
            layoutParams.height = (int) animation.getAnimatedValue();
            headView.setLayoutParams(layoutParams);
        });
        headRelaseAnimator.addListener(new Animator.AnimatorListener() {
            @Override
    ...
    private int flingCurrentHeight;
    private boolean isHeadAnimation;
    ...
    public void onTopEdgeOverFling(float overHeight, int duration) {
        ...
        if (headView == null) {
            ...
            flingCurrentHeight = headView.getHeight();
            mOverScrollEdge = OVERSCROLL_EDGE_TOP;
            ValueAnimator headFlingAnimator = ValueAnimator.ofInt(flingCurrentHeight, (int) (flingCurrentHeight + overHeight));
            headFlingAnimator.setDuration(duration);
            headFlingAnimator.setInterpolator(mInterpolator);
            headFlingAnimator.addUpdateListener(animation -> {
                ViewGroup.LayoutParams layoutParams = headView.getLayoutParams();
                layoutParams.height = (int) animation.getAnimatedValue();
                headView.setLayoutParams(layoutParams);
            });
            headFlingAnimator.addListener(new Animator.AnimatorListener() {
                @Override
                public void onAnimationStart(Animator animation) {
                    isHeadAnimation = true;
                }
                ...
                @Override
                public void onAnimationEnd(Animator animation) {
                    final int height = headView.getHeight();
                    if (height > headOriginHeight) {
                        ValueAnimator headFlingBackAnimator = ValueAnimator.ofInt(height, headOriginHeight);
                        headFlingBackAnimator.setDuration(DURATION_SPRING_BACK);
                        headFlingBackAnimator.setInterpolator(mInterpolator);
                        headFlingBackAnimator.addUpdateListener(animation1 -> {
                            ViewGroup.LayoutParams layoutParams = headView.getLayoutParams();
                            layoutParams.height = (int) animation1.getAnimatedValue();
                            headView.setLayoutParams(layoutParams);
                        });
                        headFlingBackAnimator.addListener(new Animator.AnimatorListener() {
                            @Override
                            public void onAnimationStart(Animator animation) {
                                isHeadAnimation = true;
                            }
    ...
    protected class OverFlyingDetector extends com.sunsh.baselibrary.widgets.overscrollview.listener.OverFlyingDetector {
        ...
        public OverFlyingDetector() {
            super(NestedHeadOverScrollView.this, NestedHeadOverScrollView.this);
        }
